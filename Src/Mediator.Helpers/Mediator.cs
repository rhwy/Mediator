...
namespace Mediator
{
    ...
    public static class MediatorBus
    {
        ...
        //BUFFER
        public static IEnumerable GetMessages<T>(string name)
        {
            EventBus<T> bus = EventBusManager.Current.GetFor<T>(name);
            IEnumerable messages = bus.GetBuffer();
            return messages;
        }
        public static IEnumerable GetMessages<T>()
        {
            EventBus<T> bus = EventBusManager.Current.GetFor<T>();
            IEnumerable messages = bus.GetBuffer();
            return messages;
        }
        public static IEnumerable GetMessages(string name)
        {
        ...
        //PUSH
        public static void Send<T>(object sender, string name, T message)
        {
            EventBus<T> bus = EventBusManager.Current.GetFor<T>(name);
            bus.OnNewItem(sender, message);
        }

        public static void Send<T>(object sender,T message)
        {
            EventBus<T> bus = EventBusManager.Current.GetFor<T>();
            bus.OnNewItem(sender, message);
        }
